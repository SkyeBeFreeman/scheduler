...
func (s *SchedulerTestSuite) TestForceReserve(c *check.C) {
	...
	_, err = scheduler.ReserveResources("1", true, []ResourceRequest{AmountBasedResourceRequest{Amount: 10, Resource: "memory"}}, 0)
...
func (s *SchedulerTestSuite) TestBadResourceReservation(c *check.C) {
	...
	// host doesn't exist
	_, err = scheduler.ReserveResources("2", false, []ResourceRequest{AmountBasedResourceRequest{Amount: 1, Resource: "memory"}}, 0)
	c.Assert(err, check.IsNil)
	...
	_, err = scheduler.ReserveResources("1", false, []ResourceRequest{AmountBasedResourceRequest{Amount: 1, Resource: "network"}}, 0)
	c.Assert(err, check.IsNil)
	...
	_, err = scheduler.ReserveResources("1", false, []ResourceRequest{AmountBasedResourceRequest{Amount: 1, Resource: "storage.size"}, AmountBasedResourceRequest{Amount: 1, Resource: "memory"}}, 0)
	c.Assert(err, check.IsNil)
	_, err = scheduler.ReserveResources("1", false, []ResourceRequest{AmountBasedResourceRequest{Amount: 1, Resource: "storage.size"}, AmountBasedResourceRequest{Amount: 4, Resource: "memory"}}, 0)
...
func (s *SchedulerTestSuite) TestPortReservation(c *check.C) {
	...
	data, err := scheduler.ReserveResources("1", false, []ResourceRequest{PortBindingResourceRequest{InstanceID: string(id), ResourceUUID: "u" + string(id), Resource: "portReservation", PortRequests: specs}}, 0)
	c.Assert(err, check.IsNil)
	...
	_, err = scheduler.ReserveResources("1", false, []ResourceRequest{PortBindingResourceRequest{InstanceID: string(id), ResourceUUID: "u" + string(id), Resource: "portReservation", PortRequests: specs}}, 0)
	...
	specs = []PortSpec{{IPAddress: "192.168.1.5", PublicPort: 8085, PrivatePort: 8085, Protocol: "tcp"}}
	_, err = scheduler.ReserveResources("1", false, []ResourceRequest{PortBindingResourceRequest{InstanceID: "2", ResourceUUID: "12346", Resource: "portReservation", PortRequests: specs}}, 0)
	c.Assert(err, check.NotNil)
}
...
func (s *SchedulerTestSuite) TestPortReservationProtocol(c *check.C) {
	...
	data, err := scheduler.ReserveResources("1", false, []ResourceRequest{PortBindingResourceRequest{InstanceID: string(id), ResourceUUID: string(id), Resource: "portReservation", PortRequests: specs}}, 0)
	c.Assert(err, check.IsNil)
	...
	data, err = scheduler.ReserveResources("1", false, []ResourceRequest{PortBindingResourceRequest{InstanceID: "2", ResourceUUID: "u" + string(id), Resource: "portReservation", PortRequests: specs}}, 0)
	c.Assert(err, check.IsNil)
	c.Assert(data["portReservation"].([]map[string]interface{})[0]["allocatedIPs"], check.HasLen, 2)
	id++
	...
	data, err = scheduler.ReserveResources("1", false, []ResourceRequest{PortBindingResourceRequest{InstanceID: "3", ResourceUUID: "u" + string(id), Resource: "portReservation", PortRequests: specs}}, 0)
	c.Assert(err, check.NotNil)
	id++
...
func (s *SchedulerTestSuite) TestReservePortsWithGhostIPEntry(c *check.C) {
	...
	_, err = scheduler.ReserveResources("1", false, rrequests, 0)
	c.Assert(err, check.NotNil)
...
func (s *SchedulerTestSuite) TestReleaseNoop(c *check.C) {
	...
	_, err = scheduler.ReserveResources("1", false, r1, 0)
	c.Assert(err, check.IsNil)
	...
	_, err = scheduler.ReserveResources("1", false, r3, 0)
	c.Assert(err, check.IsNil)
...
func (s *SchedulerTestSuite) TestPortCornerCase(c *check.C) {
	...
	_, err = scheduler.ReserveResources("2", false, r2, 0)
	c.Assert(err, check.IsNil)
	...
	_, err = scheduler.ReserveResources("2", false, r3, 0)
	c.Assert(err, check.NotNil)
...
func checkPrioritizationAndReserve(scheduler *Scheduler, tests []rezTest, c *check.C) {
	for _, t := range tests {
		...
		if len(t.expected) > 0 {
			_, err = scheduler.ReserveResources(t.expected[0], t.force, t.resourceRequests, 0)
			c.Assert(err, check.IsNil)
		}
	}
}
