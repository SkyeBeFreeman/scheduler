...
func (s *MetadataTestSuite) TestWatchMetadata(c *check.C) {
	...
	_, err = sched.ReserveResources("host-b", false, []scheduler.ResourceRequest{scheduler.AmountBasedResourceRequest{Amount: 2, Resource: "memoryReservation"}}, 0)
	c.Assert(err, check.IsNil)
...
func (s *MetadataTestSuite) TestIPLabelChange(c *check.C) {
	...
	_, err := sched.ReserveResources("host-a", false, r1, 0)
	c.Assert(err, check.IsNil)
	_, err = sched.ReserveResources("host-a", false, r2, 0)
	c.Assert(err, check.IsNil)
}
